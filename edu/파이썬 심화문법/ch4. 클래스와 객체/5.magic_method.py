...
매직 메서드(던더(__) 메서드) : 클래스안에 정의할 수 있는 스페셜 메서드
- 특별한 상황에 호출된다. #__init__ 객체 생성시 호출,__str__ 객체를 출력할때 호출
- __이름__의 형태로 되어있다.
...
class Unit:
    ...
    # -> 객체마다 다른 값을 가지는 속성
    def __init__(self,name,hp,shield,demage): # __init__ : 매직메서드, 생성자 첫칸은 self
        self.name=name
...
#객체가 가지고 있는 메서드 및 속성 확인(리스트 형태로 반환) : dir
print(dir(probe))